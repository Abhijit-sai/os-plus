...
assert.match(actions, /review_phone/);
assert.match(actions, /warnings: row\.warnings/);
assert.match(actions, /Resolve review rows and inspect warnings/);
...
assert.match(dialog, /Create without verified phone/);
assert.match(dialog, /row\.warnings/);
assert.match(dialog, /Create saves this customer without a verified phone/);
assert.match(dialog, /Showing every review decision plus the first 200 other rows/);
