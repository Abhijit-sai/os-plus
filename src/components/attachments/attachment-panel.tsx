            ...
            <details className="rounded-md border bg-muted/10 p-3">
              ...
              <div className="mt-3 space-y-3">
                <div className="grid gap-2">
                  ...
                  <Input
                    id={`attachment-notes-${entityId}`}
                    name="notes"
                    placeholder="Fit note, design context, or why this file matters"
                  />
                </div>
                <div className="grid gap-2">
                  <Label htmlFor={`attachment-url-${entityId}`}>
                    External file URL
                  </Label>
                  <Input
                    id={`attachment-url-${entityId}`}
                    name="fileUrl"
                    type="url"
                    placeholder="https://..."
                  />
                  <p className="text-xs text-muted-foreground">
                    Use only when the file already lives elsewhere.
                  </p>
                </div>
                <label className="flex items-start gap-2 text-sm">
                  ...
                  <span>
                    <span className="block font-medium">
                      Mark safe for customer view later
                    </span>
                    <span className="text-muted-foreground">
                      Stored as a flag. Public tracking still does not show
                      attachments.
                    </span>
                  </span>
                </label>
      ...
      >
          ...
          >
            {isImageAttachment(attachment) ? (
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex aspect-[4/3] items-center justify-center bg-muted/40"
              >
        ...
        {!attachments.length ? (
          <div className="text-sm text-muted-foreground">
            No attachments yet. Add measurement photos, design references,
            bills, or production notes when they help staff avoid guessing.
          </div>
        ) : null}
      </div>
    </div>
    ...
    <div className="min-w-0">
      <div className="flex items-center gap-2">
        ...
        <p className="truncate font-medium">
          {attachment.label ?? inferDisplayType(attachment)}
        </p>
      </div>
      <div className="mt-2 flex flex-wrap gap-2">
        <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
          {inferDisplayType(attachment)}
        </span>
        {fileSize ? (
          <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
            {fileSize}
      ...
      {attachment.notes ? (
        <p className="mt-2 text-sm text-muted-foreground">{attachment.notes}</p>
      ) : null}
      {!compact ? (
