...
const navItems: Array<{
  ...
  { href: "/production", label: "Production", icon: Factory, permission: "production:view" },
  { href: "/customers", label: "Customers", icon: Users, permission: "customers:view" },
  { href: "/workers", label: "Workers", icon: Users, permission: "workers:view" },
  { href: "/attendance", label: "Attendance", icon: CalendarDays, permission: "attendance:view" },
  { href: "/salary", label: "Salary", icon: WalletCards, permission: "salary:view" },
  { href: "/finance", label: "Finance", icon: Landmark, permission: "finance:view" },
  { href: "/reports", label: "Reports", icon: LineChart, permission: "reports:view" },
...
export function AppShell({
  ...
  const visibleNavItems = navItems.filter((item) => hasPermission(context.membership.role, item.permission));
  ...
  return (
    <div className="min-h-screen bg-muted/40">
      <aside className={cn("fixed inset-y-0 left-0 hidden border-r bg-background transition-all md:flex md:flex-col", sidebarWidthClass)}>
        <div className="border-b p-3">
          <div className={cn("flex items-center", isCollapsed ? "justify-center" : "gap-3")}>
            {context.tenant.logo_url ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={context.tenant.logo_url} alt={context.tenant.store_name} className="h-10 w-10 rounded-md object-cover" />
            ) : (
              <div
                className="flex h-10 w-10 items-center justify-center rounded-md text-sm font-semibold text-white"
                style={{ backgroundColor: context.tenant.brand_color ?? "#2563eb" }}
              >
                {context.tenant.store_name.slice(0, 2).toUpperCase()}
              </div>
            )}
            <div className={cn("min-w-0", isCollapsed ? "hidden" : "")}>
              <p className="truncate font-semibold">{context.tenant.store_name}</p>
              <p className="truncate text-xs text-muted-foreground">{formatRole(context.membership.role)}</p>
            </div>
          </div>
          <button
            type="button"
      ...
      <div className={cn("transition-all", contentPaddingClass)}>
        <header className="sticky top-0 z-10 border-b bg-background/95 px-4 py-3 backdrop-blur md:px-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-muted-foreground">OS PLUS</p>
            ...
            <details className="group relative">
              <summary className="flex cursor-pointer list-none items-center gap-2 rounded-md border bg-background px-3 py-2 text-sm font-medium hover:bg-accent focus:outline-none focus:ring-2 focus:ring-ring">
                ...
                <span className="hidden max-w-40 truncate sm:inline">
                  {context.user.fullName ?? context.user.primaryEmail ?? "Account"}
                </span>
                <ChevronDown className="h-4 w-4 text-muted-foreground transition-transform group-open:rotate-180" />
              </summary>
              <div className="absolute right-0 mt-2 w-80 rounded-md border bg-background p-2 shadow-lg">
                <div className="border-b px-3 py-2">
                  <p className="truncate text-sm font-semibold">{context.user.fullName ?? "Signed in user"}</p>
                  <p className="truncate text-xs text-muted-foreground">{context.user.primaryEmail ?? context.membership.email ?? "Email unavailable"}</p>
                </div>
                <div className="space-y-2 border-b px-3 py-3">
                  <div className="flex items-start gap-2">
                    <Building2 className="mt-0.5 h-4 w-4 text-muted-foreground" />
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium">{context.tenant.store_name}</p>
                      <p className="truncate text-xs text-muted-foreground">{context.tenant.slug}</p>
                    </div>
                  </div>
                  <p className="text-xs font-medium text-muted-foreground">Role: {formatRole(context.membership.role)}</p>
                </div>
                <div className="py-1">
                  <Link href="/select-tenant" className="flex items-center gap-2 rounded-md px-3 py-2 text-sm hover:bg-accent">
